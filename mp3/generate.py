...
def main():
    ...
    try:
        with open(main_app_source, "w") as out:
            ...
            out.write("};\n\n")
            out.write(
                "unsigned int mp3FilesCount = sizeof(mp3Files) / sizeof(mp3Files[0]);\n"
            )
            ...
            if call_dict is not None:
                ...
                out.write("};\n")
                out.write("bool hasMp3ForCall(const char* callNumber) {\n")
                out.write("    for (const auto& mapping : callNumbersToMp3s) {\n")
                out.write(
                    "        if (strcmp(mapping.callNumber, callNumber) == 0) {\n"
                )
                ...
                out.write("            return mapping.fileName;\n")
                out.write("        }\n")
                out.write("    }\n")
    ...
    try:
        with open(write_header, "w") as out:
            ...
            out.write("#include <cstring>\n\n")
            out.write(
                "// Write MP3 header: Contains full MP3 data arrays and metadata\n"
            )
            out.write("// Generated by mp3_to_header.py\n\n")
    ...
    try:
        with open(write_source, "w") as out:
            out.write(
                "// Write MP3 source: Contains full MP3 data arrays and metadata\n"
            )
            out.write("// Generated by mp3_to_header.py\n\n")
            ...
            for filename, base_name in zip(mp3_files, base_names):
                array_name = base_name + "_mp3"
                out.write(
                    f'const MP3File {base_name} = {{ "{filename}", {array_name}, {array_name}_len }};\n'
                )
            out.write("\n")
