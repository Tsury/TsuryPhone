...
DialedNumberValidationResult validateDialedNumber(const char *number,
  ...
  // Check webhook entries across all servers FIRST
  else if (serverManager && serverManager->isWebhookEntry(number)) {
    res = DialedNumberValidationResult::Valid;
  }
  // Check for partial webhook entries
  else if (serverManager && serverManager->isPartialWebhookEntry(number)) {
    res = DialedNumberValidationResult::Pending;
  } else if (phoneConfig && phoneConfig->isPhoneBookEntry(number)) {
    res = DialedNumberValidationResult::Valid;
  } else if (phoneConfig && phoneConfig->isPartialOfPhoneBookEntry(number)) {
    res = DialedNumberValidationResult::Pending;
  }
