...
void IntegrationManager::updateCallInfo(const String& number, bool isIncoming, unsigned long startTime) {
  for (auto& integration : _integrations) {
...
void IntegrationManager::registerIntegrations() {
  ...
  // _integrations.push_back(std::unique_ptr<IIntegration>(new AndroidIntegration(_config, _stats)));
  // Logger::infoln(F("Registered Android integration"));
  // #endif
